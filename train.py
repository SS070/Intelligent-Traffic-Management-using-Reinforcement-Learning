...
def run(train=True, model_name="models", epochs=50, steps=500):
    ...
    brain = Agent(0.99, 1.0, 0.1, 4, 256, 256, 1024, 4, junction_numbers)
    ...
    for e in range(epochs):
        if train:
            ...
            traci.start([checkBinary("sumo-gui"), "-c", "configuration.sumocfg", "--tripinfo-output", "maps/city3.rou.xml"])
        ...
        for junction_number, junction in enumerate(all_junctions):
            ...
            all_lanes.extend(traci.trafficlight.getControlledLanes(junction))
    ...
    if train:
        ...
        # 1. Total waiting time vs Epochs
        plt.figure()
        ...
        # Plot 4: Explore vs Exploit Ratio
        plt.figure()
